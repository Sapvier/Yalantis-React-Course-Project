...
export const fetchMyProducts = async (currentPage, perPage, filter) => {
    let origin = []
    filter.origin.map(item => {
        if (item.isChecked) {
            origin.push(item.value)
    ...
    const response = await fetch(`https://yalantis-react-school-api.yalantis.com/api/v1/products?page=${currentPage}&perPage=${perPage}&origins=${origin.join()}&minPrice=${filter.price.minPrice}&maxPrice=${filter.price.maxPrice}&editable=true`, {
        method: 'GET',
        headers: {
...
export const fetchOrder = async (location) => {
    const response = await fetch(`https://yalantis-react-school-api.yalantis.com/api/v1${location.pathname}`)
    return response.json()
}